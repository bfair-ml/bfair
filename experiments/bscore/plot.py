...
with open(file_path, "r") as file:
    for line in file:
        ...
        for model, metrics in data.items():
            ...
            count_disparity_stds.append(metrics["count_disparity"]["standard_deviation"])  # Store standard deviation
            log_score_means.append(metrics["log_score"]["mean"])
            log_score_stds.append(metrics["log_score"]["standard_deviation"])  # Store standard deviation
...
axes[0].bar(models, count_disparity_means, yerr=count_disparity_stds, capsize=5, color='skyblue', label='Mean ± Std Dev')
...
# Plot log_score
axes[1].bar(models, log_score_means, yerr=log_score_stds, capsize=5, color='salmon', label='Mean ± Std Dev')
