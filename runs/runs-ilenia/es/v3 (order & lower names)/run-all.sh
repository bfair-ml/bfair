python -m experiments.bscore --context continuous --dataset ilenia --export-csv "runs-ilenia/aggregation.csv" --use-root yes --lower-proper-nouns yes --semantic-check no --split-endings no --inspect-professions no --use-morph no >> "runs-ilenia/.log.txt"
