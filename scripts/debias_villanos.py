...
for violent, group_by_violent in ds.data.groupby(LABEL_COLUMN):
    for category, count in group_by_violent[OBJECTIVE].value_counts().items():
        sizes[category] = min(sizes[category], count)
sizes = dict(sizes)
print(sizes)
...
for violent, group_by_violent in ds.data.groupby(LABEL_COLUMN):
    ...
    for objective, group_by_objective in group_by_violent.groupby(OBJECTIVE):
        print(list(objective), len(group_by_objective), "->", sizes[objective])
        selected.append(
...
selected_df.to_csv(f"villanos_debiased_by_{OBJECTIVE.lower()}.csv", sep=";", index=False)
